...
{ id:'scholarship_result', category:'🏫 School', minAge:12, maxAge:12, type:'story',
  ...
  { id:'first_crush', category:'💌 Social', minAge:12, maxAge:16, type:'story',
    traitTags:['social_event','romance_event'],
    text:"You've developed feelings for someone in your class. They glance at you in maths and look away too quickly.",
    choices:[
      { text:'Pass them a note',
        effects:{ happy:+8, rep:+3 },
        log:'Passed a note to your crush' },
      { text:'Ask a friend to find out if they like you',
        effects:{ happy:+5 },
        log:'Asked a friend about your crush' },
      { text:'Say nothing — too risky',
        effects:{ happy:-3 },
        log:'Said nothing to your crush' },
    ]},
  ...
  { id:'uni_decision', category:'🎓 Education', minAge:18, maxAge:19, type:'story',
    traitTags:['academic_event','career_opportunity'],
    text:"A-levels are done. The next chapter is yours to write. University, work, or something else?",
    choices:[
      { text:'Apply to a top-tier university',
        effects:{ smarts:+15, happy:+8, balance:-12000, rep:+10 },
        log:'Left home for a top university' },
      { text:'Go to a local university and save money',
        effects:{ smarts:+10, balance:-5000 },
        log:'Studied at a local university' },
      { text:'Skip uni and go straight into work',
        effects:{ balance:+12000, smarts:-5 },
        log:'Skipped university and entered the workforce' },
    ]},

  { id:'first_job', category:'💼 Work', minAge:21, maxAge:26, type:'story',
    traitTags:['career_opportunity'],
    text:'Two job offers arrive at the same time. Same starting salary. Very different futures.',
    choices:[
      { text:'Take the corporate job — stable and safe',
        effects:{ income:+28000, happy:-3, rep:+5 },
        log:'Joined a corporate firm' },
      { text:'Join the startup — risky but exciting',
        effects:{ income:+22000, smarts:+10, happy:+8 },
        log:'Joined an early-stage startup' },
      { text:'Negotiate both against each other',
        effects:{ income:+33000, rep:+8 },
        log:'Negotiated a better offer' },
    ]},

  { id:'heartbreak', category:'💔 Social', minAge:18, maxAge:30, type:'story',
    traitTags:['social_event','romance_event'],
    text:"Your long-term relationship ends. You didn't see it coming. Nobody ever does.",
    choices:[
      { text:'Throw yourself into work',
        effects:{ smarts:+8, balance:+2000, happy:-12 },
        log:'Buried yourself in work after heartbreak' },
      { text:'Lean on friends and let yourself grieve',
        effects:{ happy:+5, rel_friends:+10 },
        log:'Leaned on friends after a breakup' },
      { text:'Move city for a fresh start',
        effects:{ happy:+8, balance:-3000 },
        log:'Moved city after a breakup' },
    ]},

  { id:'property', category:'🏠 Finance', minAge:25, maxAge:40, type:'story',
    traitTags:['financial_event'],
    text:"The property ladder. Everyone says you should buy. The deposit alone makes you feel sick.",
    choices:[
      { text:'Buy — stretch the budget',
        effects:{ balance:-15000, happy:+10, rep:+5 },
        log:'Bought your first property' },
      { text:'Wait another year and keep saving',
        effects:{ balance:+4000 },
        log:'Chose to keep saving before buying' },
      { text:'Buy with a friend to split costs',
        effects:{ balance:-8000, happy:+8 },
        log:'Bought a property with a friend' },
    ]},

  { id:'health_scare', category:'🏥 Health', minAge:35, maxAge:65, type:'story',
    traitTags:['health_event'],
    text:"A routine checkup flags something unexpected. The doctor wants further tests.",
    choices:[
      { text:'Follow every step — better to know',
        effects:{ health:-3, happy:-5, smarts:+3 },
        log:'Dealt with a health scare properly' },
      { text:'Do the tests but try not to spiral',
        effects:{ health:-2, happy:-2 },
        log:'Got through a health scare' },
      { text:'Ignore it — you feel absolutely fine',
        effects:{ health:-12, happy:+3 },
        log:'Ignored a health warning' },
    ]},
  ...
  { id:'rand_illness', category:'🤒 Health', minAge:5, maxAge:80, type:'random',
    traitTags:['health_event'],
    text:"You come down with a nasty illness and need weeks to recover.",
    choices:[
      { text:'Rest and recover properly',
        effects:{ health:-5, happy:-3 },
        log:'Got ill and rested properly' },
      { text:'Push through — no time for this',
        effects:{ health:-12, smarts:-3 },
        log:'Pushed through an illness' },
    ]},

  { id:'rand_opportunity', category:'✨ Luck', minAge:20, maxAge:60, type:'random',
    traitTags:['career_opportunity'],
    text:"A contact recommends you for a freelance project out of nowhere.",
    choices:[
      { text:'Take it on',
        effects:{ balance:+1500, smarts:+5, happy:+3 },
        log:'Took on an unexpected freelance project' },
      { text:'Pass — too much on right now',
        effects:{ happy:+2 },
        log:'Turned down a freelance opportunity' },
    ]},

  { id:'rand_argument', category:'💥 Social', minAge:8, maxAge:80, type:'random',
    traitTags:['social_event'],
    text:"You have a significant falling out with someone close to you.",
    choices:[
      { text:'Apologise — even if it wasn\'t your fault',
        effects:{ happy:+3, rel_family:+8, rep:+5 },
        log:'Apologised first after a big argument' },
      { text:'Stand your ground',
        effects:{ happy:-5, rel_family:-10 },
        log:'Held firm in an argument' },
      { text:'Give it time and space',
        effects:{ happy:-2, rel_family:-3 },
        log:'Let an argument cool down on its own' },
    ]},
];
